...
import { TRANSLATOR_PASSWORD } from '$env/static/private';
import { createSession } from '../../../hooks.server';
import { z } from 'zod';
import { timingSafeEqual } from 'crypto';
import { checkRateLimit, recordLogin } from '$lib/server/rateLimiter';
...
export const load = ({ locals }) => {
	if (locals.user?.role === 'admin' || locals.user?.role === 'translator') {
...
export const actions = {
	default: async ({ cookies, request }) => {
		...
		if (!safeCompare(password, TRANSLATOR_PASSWORD)) {
			recordLogin(ip, false, 'translator');
			return fail(401, { errors: { password: ['Invalid password'] }, password });
		}

		recordLogin(ip, true, 'translator');

		const session = createSession('translator', 'translator');
